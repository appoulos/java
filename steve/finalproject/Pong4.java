...
public class Pong4 extends JPanel implements ActionListener, KeyListener {
    ...
    public Pong4() {
        JFrame frame = new JFrame("Welcome to a game of Pong - First to 10");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (sPressed && player1Y < getHeight() - paddleHeight) player1Y += 5;
        if (upPressed && player2Y > 0) player2Y -= 5;
        if (downPressed && player2Y < getHeight() - paddleHeight) player2Y += 5;

        // AI movement if not two-player
        if (!twoPlayerMode) {
            if (player2Y + paddleHeight / 2 < ballY) player2Y += 3;
            else if (player2Y + paddleHeight / 2 > ballY) player2Y -= 3;
        }
